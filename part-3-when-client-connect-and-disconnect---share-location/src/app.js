...
io.on("connection",(socket)=>{// this function will run when every new client get connected to server and on function is used to listen and react on same event And in order call this function from frontend you have to use io()
    ...
    socket.emit("message",serverMessage);//Now when user get connected i want to send new event to client and emit function is used to register new event
    socket.broadcast.emit("message","A new user has joind!")//this will send message to every connected client except that particular client or socket

    socket.on('sendMessage',(message)=>{
        // socket.emit("countUpdated",count); for single client or particular connection
        io.emit("message",message);//emitting to every connection or every client
    ...
    socket.on("disconnect",()=>{
        socket.broadcast.emit("message","A get dis connected!"); 
